...
}var I=$j.ajax({url:A}).fail(function(K,L){alert("Server error. Please contact your administrator. The status of the error is : "+K.status)
...
return false
}function sVF(B,F,A,D,E){row=$j("#createViolationForm"+A);
if(!row.length){expandAccordionItem(B);
var C=$j(B).closest(".pos"+A);
$j.get(baseUrl+"/resource/show_create_violation_form?resource="+F+"&line="+A+"&gray_colspan="+D+"&white_colspan="+E,function(G){C.after(G)
}).error(function(){alert("Server error. Please contact your administrator.")
})
}return false
}function hVF(B,A){var C=$j(B).closest("#createViolationRow"+A);
